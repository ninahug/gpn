...
def apply_2opt(tup):
	...
	for i, route in enumerate(new_routes, 1):
		...
		for j in range(len(route)-1):
			distance = np.linalg.norm(xy[j] - xy[j+1])
			print("distance", distance)
			arravial_time = route_duration + np.clip((tw[j][0] - route_duration), 0, 1000)
			print("arravial_time",arravial_time)
			time_cost += np.abs((1 / 2 * (tw[j][0] - tw[j][1])) - arravial_time)*0.01
			print("time_cost", time_cost)
			route_duration += distance + 0.1
		...
		fuel_pay = fuel_cost.sum(0)*57.3*(1/capitity)
		print("fuel_pay", fuel_pay)
		# self.cur_time = arrival_time.squeeze(-1)
		# time_cost = torch.abs((1 / 2 * (start - end)) - arrival_time)
		# arrival_time = kk[:, :, None] + dist + service + ((start - kk[:, :, None]).clamp_(min=0))
		# self.time_cost = time_cost.squeeze(-1)
		# route_duration += data['dist_mat'][idx_from][idx_to] + 10
		# arravial_time = route_duration + np.clip((data['tw'][idx_from][0] - route_duration), 0, 1000)
		# time_cost = np.abs((1 / 2 * (data['tw'][idx_from][0] - data['tw'][idx_from][1])) - arravial_time)
		# print("fuel_pay", fuel_pay)

		total_length = np.sum(lengths)
		pay_cost = total_length*6
...
def transfer_data(data1, num_samples=1, batch_size=1):
	datas = []
	device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
	for i in range(num_samples):
		citys =  np.concatenate((data1['depot_xy'].cpu().numpy(), data1['customer_xy'].cpu().numpy()))
		edges_index = data1['edges_index'].cpu().numpy()
		edges = data1['edges'].cpu().numpy()
		n_demand = data1['demand'].cpu().numpy()
		tw = data1['tw'].cpu().numpy()
		n_durations = data1['durations'].cpu().numpy()

		capcity = data1['car_capacity'].cpu().numpy()
		car_start_node = data1['car_start_node'].cpu().numpy()
		dloc = data1['depot_xy'].cpu().numpy()
		nd = len(dloc)
		d_durations = np.full([nd], 0).tolist()
		durations = np.concatenate((d_durations, n_durations))
		d_d = np.full([nd], 0).tolist()
		demand = np.concatenate((d_d, n_demand))

		nloc = data1['customer_xy'].cpu().numpy()
		data = Data(x=torch.tensor(citys).to(device), edge_index=torch.tensor(edges_index).to(device),
						edge_attr=torch.tensor(edges).to(device),
						demand=torch.tensor(demand).unsqueeze(-1).to(device), tw=torch.tensor(tw).to(device),
						durations=torch.tensor(durations).unsqueeze(-1).to(device),
						capcity=torch.tensor(capcity).unsqueeze(-1).to(device), car_start_node=torch.tensor(car_start_node).to(device),
						nloc=torch.tensor(nloc).to(device), dloc=torch.tensor(dloc).to(device))

		datas.append(data)
		dl = DataLoader(datas, batch_size=1)
		#
		# print("dl",next(iter(dl)))
		return dl
...
if __name__ == '__main__':
	...
	if args.txt is not None:
		...
		data1 = hoge.load_json(device)  # return tensor on GPU
		for k, v in data1.items():
			...
			# print("shape",shape)
			data1[k] = v
		data = transfer_data(data1, num_samples=4, batch_size=4)
	pretrained = pretrained.to(device)
	# data = list(map(lambda x: x.to(device), data))
	pretrained.eval()
	with torch.no_grad():
		for t, inputs in enumerate(data):
			costs, _, pis = pretrained(inputs, return_pi=True, decode_type='greedy')

			tup = get_more_info(inputs,costs, pis[0].cpu().numpy(), 1)
			...
			print(
				f'decode type: {args.decode_type}\nminimum cost: {cost:.3f}\nidx: {1} out of {args.batch} solutions')
