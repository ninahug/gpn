...
from formats import RPInstance, RPObs
...
def generate_cvrptw_data(size, graph_size, rnds=None,
	...
	# 		"demand": torch.tensor(np.minimum(np.maximum(np.abs(rnds.normal(loc=15, scale=10, size=[size, graph_size])).astype(int), 1), 42).tolist(), device = device),
	# 		"capacity": torch.tensor(np.full(size, TW_CAPACITIES[graph_size]).tolist(), device = device),
	# 		"depot_tw": torch.tensor([[0, service_window]] * size, device = device),
    #         "node_tw":torch.tensor(tw, device = device),
    #         "durations": torch.tensor(np.full([size, graph_size], service_duration).tolist(), device = device),
