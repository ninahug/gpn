...
def arg_parser():
	...
	parser.add_argument('--seed', metavar = 'SE', type = int, default = 1234, help = 'random seed number for inference(or validation during training), reproducibility')
	# main config
	parser.add_argument('-n', '--n_customer', metavar = 'N', type = int, default = 20, help = 'number of customer nodes, time sequence')
	parser.add_argument('-c', '--n_car_each_depot', metavar = 'C', type = int, default = 1, help = 'number of available vehicles per depot')
	parser.add_argument('-D', '--capa', metavar = 'CA', type = float, default = 1., help = 'initial capacity of vehicles')
	parser.add_argument('-w', '--service_window', metavar='SW', type=float, default=1000., help='service window')
	parser.add_argument('-sd', '--service_duration', metavar='SD', type=float, default=10., help='service_duration')
	parser.add_argument('-tf', '--time_factor', metavar='TF', type=float, default=100., help='time_factor')
	parser.add_argument('-te', '--tw_expansion', metavar='TP', type=float, default=3., help='tw_expansion')
	parser.add_argument('-d', '--n_depot', metavar = 'D', type = int, default = 1, help = 'number of depot nodes')

	# batch config
	parser.add_argument('-b', '--batch', metavar = 'B', type = int, default = 3, help = 'batch size')
	parser.add_argument('-s', '--batch_steps', metavar = 'S', type = int, default = 2, help = 'number of samples = batch * batch_steps')
	parser.add_argument('-v', '--batch_verbose', metavar = 'V', type = int, default = 5, help = 'print and logging during training process')
...
def test_parser():
	...
	parser.add_argument('-n', '--n_customer', metavar = 'N', type = int, default = 20, help = 'number of customer nodes, time sequence')
	parser.add_argument('-c', '--n_car', metavar = 'C', type = int, default = 1, help = 'number of available vehicles')
	parser.add_argument('-d', '--n_depot', metavar = 'D', type = int, default = 1, help = 'number of depot nodes')
	parser.add_argument('-s', '--seed', metavar = 'S', type = int, default = 1234, help = 'random seed number for inference, reproducibility')
