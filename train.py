...
# import sys
# sys.path.append('./')
from plot import get_more_info,apply_2opt
...
def train(cfg):
	...
	# critic = StateCritic(cfg.embed_dim,cfg.n_heads,cfg.n_encode_layers)
	model.to(device)
	# critic.to(device)
	...
	validation_dataset = cvrptw(device, size = cfg.n_val_samples,n_customer=cfg.n_customer,seed=cfg.seed,
				   n_depot=cfg.n_depot, n_car=cfg.n_car_each_depot, service_window=cfg.service_window, service_duration=cfg.service_duration,
				   time_factor=cfg.time_factor, tw_expansion=cfg.tw_expansion)
	...
	def rein_loss(model, inputs, bs, t, device):
		...
		#L是通过AttentionModel以及config中的model直接计算出来的
		b = bs[t] if bs is not None else baseline.eval(inputs, L)
		#b是通过将weight中的权重加载到RolloutBaselin中得到的 基线成本和exp移动平均成本的组合。
	...
	for epoch in range(cfg.epochs):
		...
		dataset = cvrptw(device, size =  cfg.n_samples,n_customer=cfg.n_customer,seed=cfg.seed,
				   n_depot=cfg.n_depot, n_car=cfg.n_car_each_depot, service_window=cfg.service_window, service_duration=cfg.service_duration,
				   time_factor=cfg.time_factor, tw_expansion=cfg.tw_expansion)
